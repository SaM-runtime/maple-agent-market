...
function Save-MapleJsonFile {
    ...
    $temporary = Join-Path $parent ('.' + [System.IO.Path]::GetFileName($fullPath) + '.tmp-' + [guid]::NewGuid().ToString('N'))
    $backup = $fullPath + '.bak'
...
function Import-MapleSkinFolder {
    ...
    $contentHash = Get-MapleCompositeHash -Files $required
    ...
    $staging = Assert-MapleChildPath -Root $importsRoot -Candidate (Join-Path $importsRoot ('.staging-' + [guid]::NewGuid().ToString('N')))
...
function Copy-MapleSkinToSlot {
    ...
    if ($SkinId -match '^builtin-(?<slot>[0-7])$') {
        ...
        for ($pose = 0; $pose -lt 3; $pose++) {
            ...
            Copy-Item -LiteralPath (Join-Path $BasePack ('training_avatar_attack_hires_{0}.sprite' -f ($sourceSlot * 3 + $pose))) -Destination (Join-Path $DestinationPack ('training_avatar_attack_hires_{0}.sprite' -f ($TargetSlot * 3 + $pose))) -Force
        }
    ...
    # skin's own stand1 frames keeps identity/scale stable without fabricating
    # a sit or alert drawing that the user did not supply.
    for ($pose = 0; $pose -lt 3; $pose++) {
        $fallback = Join-Path $sourceRoot ('stand-{0}.sprite' -f $pose)
        Copy-Item -LiteralPath $fallback -Destination (Join-Path $DestinationPack ('market_avatar_stand2_hires_{0}.sprite' -f ($TargetSlot * 3 + $pose))) -Force
        Copy-Item -LiteralPath $fallback -Destination (Join-Path $DestinationPack ('market_avatar_alert_hires_{0}.sprite' -f ($TargetSlot * 3 + $pose))) -Force
    }
    Copy-Item -LiteralPath (Join-Path $sourceRoot 'stand-0.sprite') -Destination (Join-Path $DestinationPack ('market_avatar_sit_hires_{0}.sprite' -f $TargetSlot)) -Force
}
...
function New-MapleResolvedSkinPack {
    ...
    $staging = Assert-MapleChildPath -Root $root -Candidate (Join-Path $root ('.staging-' + [guid]::NewGuid().ToString('N')))
    try {
