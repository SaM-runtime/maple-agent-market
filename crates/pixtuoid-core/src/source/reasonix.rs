...
//! v1.0.0+) ships a CC-style hook system; `maple-agent-market connect reasonix`
//! registers the shim in the GLOBAL `~/.reasonix/settings.json`
...
pub fn decode_rx_hook_payload(v: &Value) -> Result<Vec<AgentEvent>> {
    ...
    // Mirrors the SessionStart arm: no upstream session id exists; the cwd IS
    // the session key.
